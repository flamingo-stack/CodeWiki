...
Job Processing is a child module of the [Frontend Core](../../frontend-core.md) module. It sits between the HTTP-facing request layer and the heavier backend documentation-generation pipeline:
...
- The backend documentation pipeline (`DocumentationGenerator`) and the shared `Config` object are invoked directly by `BackgroundWorker` to perform the actual analysis and generation work; see [Backend Core](../../backend-core.md)'s [Documentation And Services](../../backend-core/documentation-and-services/documentation-and-services.md) sub-module and [Config Core](../../config-core.md) for details on that pipeline.
...
- `JobStatus` – tracks `job_id`, `repo_url`, `status` (`queued`/`processing`/`completed`/`failed`), timestamps, `progress` text, `error_message`, `docs_path`, and `main_model`.
...
stateDiagram-v2
    [*] --> queued : add_job()
    queued --> processing : _process_job() starts
...
- **[Request Handling](../request_handling/request_handling.md)** — the consumer of `BackgroundWorker`'s public interface (`add_job`, `get_job_status`, `get_all_jobs`).
- **[GitHub Integration](../github_integration/github_integration.md)** — supplies `GitHubRepoProcessor`, used by `BackgroundWorker` to clone repositories and resolve repository metadata.
- **[Configuration And Data Models](../configuration_and_data_models/configuration_and_data_models.md)** — supplies `JobStatus`, `CacheEntry`, and `WebAppConfig`.
- **[Backend Core](../../backend-core.md)** and **[Config Core](../../config-core.md)** — provide `DocumentationGenerator` and `Config` respectively, used directly by `BackgroundWorker` to execute the documentation generation pipeline.
