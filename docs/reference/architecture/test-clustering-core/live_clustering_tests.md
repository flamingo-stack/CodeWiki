...
This lightweight, duplicated pattern avoids introducing a shared test-utility dependency for what are meant to be quick, disposable debugging scripts. Note that a more structured, reusable implementation of similar reporting logic exists in the sibling [Validation And Logic Tests](../validation_and_logic_tests/validation_and_logic_tests.md) module, and a similar pattern also appears in the unrelated [Test Multi Path Core](../../test-multi-path-core.md) module.
