...
This module is a child of the [CLI Core](../cli_core.md) module and is consumed by sibling modules such as [Generation](../generation/generation.md), [Configuration](../configuration/configuration.md), [Utils](../utils/utils.md), and other CLI orchestration components that need to create, update, or persist job records.
...
- **[CLI Core](../cli_core.md)** (parent module): Exposes `DocumentationJob`, `GenerationOptions`, `JobStatistics`, `JobStatus`, and `LLMConfig` as part of its public component surface, making them available to all CLI subsystems.
