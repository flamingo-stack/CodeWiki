...
- **Shared filesystem helpers** — `safe_read` / `safe_write` provide atomic, encoding-safe file I/O used to load the template/JSON files and write the final HTML output. See the [Utils](../utils/utils.md) module for other shared CLI utilities such as logging and progress tracking.
- **`FileSystemError`** — raised when the template file is missing or when reading/writing fails, allowing the CLI layer to surface a consistent error type.
...
Html Generation is invoked as an optional, final stage by `CLIDocumentationGenerator` (from the [Generation](../generation/generation.md) module), which drives the overall CLI workflow: dependency analysis → module clustering → documentation generation → **HTML generation** → job finalization.
